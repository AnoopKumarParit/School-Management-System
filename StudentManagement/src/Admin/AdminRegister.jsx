  ...
  let [state,setState]=useState({
      ...
      // }
       ...
       let handleSubmit=async(e)=>{
              ...
              try{
                  console.log(state);
                  let x=await axios.post("http://localhost:8080/saveAdmin",state)
                  console.log(x);
                  alert(x.data.msg)
                  setState({
                       fname:"",
    ...
    return(
      ...
      <div className='flex flex-col p-[10px] m-[auto] w-[300px]'>
          <form action="" onSubmit={(e)=>{handleSubmit(e)}} className="max-w-md mx-auto mt-10 bg-white shadow-xl rounded-2xl p-6 space-y-4">
              ...
              <input required value={state.phoneNumber} type="text" name="phoneNumber" className='w-[300px] mt-[10px] rounded text-2xl p-[10px] border-3' onChange={e=>(setState({...state,[e.target.name]:e.target.value}))} placeholder='Enter phoneNumber' />
              <button  className="w-[300px] bg-green-600 text-white font-medium py-3.5 rounded-md hover:bg-blue-700 transition-colors p-[10px]">Submit</button>
          </form>
